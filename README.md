...
-   [주요 기능](#주요기능)
-   [트러블 슈팅](#트러블슈팅)
    -   [피크 타임 서버 부하 문제 해결](#피크-타임-서버-부하-문제-해결)
...
전국 200여 개 지점, 각 지점별 여러 반의 학생 등하원 시간(오전 8시, 오후 10시)에 데이터가 몰리면서 서버 부하 발생으로 시스템 성능 저하 및 응답 지연 발생.

**해결 방안:**

1. **서버 스케일링 아웃 및 성능 향상:**
    - 서버 인스턴스를 추가로 배포하여 부하를 분산하거나, EC2 인스턴스의 성능을 높이는 방안을 고려.
2. **부하 성능 테스트 진행 (Artillery):**
    - 등하원 데이터 Insert 트랜잭션에 대한 부하 테스트를 진행하여 실제 트래픽 처리 능력을 사전 검증.
    - 초기 테스트 환경(t3.micro 인스턴스)에서 이슈 미발생 확인.
3. **테스트 환경 설정 및 업그레이드 협의:**
    - 테스트 환경과 동일한 t3.micro 인스턴스를 사용하여 배포 진행.
    - 클라이언트와 협의하여 이슈 발생 시 서버 인스턴스 업그레이드 결정.

**결론:**

위와 같은 해결 방안을 통해 문제가 될 수 있는 부분을 사전에 테스트하여 시스템 안정성을 확보 하게 되었습니다. 또한, 저사양에 대한 테스트 및 배포를 통해 서버 비용 절감을 하게 되었습니다.
